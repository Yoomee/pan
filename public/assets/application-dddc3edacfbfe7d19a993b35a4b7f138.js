...
(function() {
  ...
  window.YmCore = {
    ...
    scrollTo: function(elem, options) {
      elem = $(elem);
      if (elem.length) {
        options = YmCore.addDefaults(options, {
          offset: 0,
          duration: 750
        });
    ...
    Tabs: {
      init: function() {
        ...
        if (window.location.hash) {
          ...
          if (target_tab_link.length) target_tab_link.tab('show');
        }
        return $('.tab-pane').has('input.error, .control-group.error').each(function(idx, pane) {
          var link;
          link = $(".tabbable .nav a[href='#" + ($(pane).attr('id')) + "']");
          link.parent().addClass('error');
          if (idx === 0) return link.tab('show');
        });
      }
    },
    Bootstrap: {
      init: function() {
        return $("[data-toggle='modal'][data-modal-url]").live('click', function() {
          var new_modal;
    ...
    Forms: {
      ...
      init: function() {
        $(".formtastic:not('.loading-text-disabled')").live("submit", function() {
          var loadingText, submitBtn;
          submitBtn = $(this).find("input[type='submit']");
          loadingText = submitBtn.data("loading-text") || 'Saving...';
          return submitBtn.addClass('disabled').val(loadingText);
        });
        return YmCore.Forms.initDatepickers();
      }
    },
    Modals: {
    ...
    ReadMoreTruncate: {
      init: function() {
        return $('.read-more-link').live('click', function(event) {
          var wrapper;
          event.preventDefault();
    ...
    init: function() {
      ...
      return YmCore.ReadMoreTruncate.init();
    }
  };
...
(function() {
  ...
  window.EmbeddableMedia = {
    ...
    embedCode: function(url) {
      var ec, host, host_matches, url_matches;
      ec = "";
      host_matches = url.match(/https?:\/\/(www\.)?([^:\/?#\.]*)/);
      if ((host_matches != null) && (host = EmbeddableMedia.hosts[host_matches[2]])) {
        url_matches = url.match(host.regex);
        if (url_matches) return ec = host.embed.replace('#ID#', url_matches[3]);
      }
    }
  };
  ...
  window.Redactor = {
    init: function() {
      return $('textarea.redactor').redactor({
        ...
        removeStyles: true,
        convertDivs: true,
        buttons: ['bold', 'italic', 'formatting', '|', 'unorderedlist', 'orderedlist', '|', 'link', 'image', 'media', 'file', 'html', 'fullscreen'],
        buttonsCustom: {
          media: {
            title: RLANG.media,
            func: 'showMedia'
        ...
        modal_media: "        <form id='redactorInsertVideoForm'>          <label>" + RLANG.media_url + "</label>          <input type='text' id='redactor_insert_media_url_input' style='width: 99%;'>          <a href='#' data-toggle='collapse' data-target='#redactorMediaAdvanced'>Advanced options</a>          <div id='redactorMediaAdvanced' class='collapse'>            <label>" + RLANG.media_html_code + "</label>            <textarea id='redactor_insert_media_html_area' style='width: 99%; height: 120px;'></textarea>          </div>        </form>        <div id='redactor_modal_footer'>          <span class='redactor_btns_box'>            <input type='button' id='redactor_insert_media_btn' class='btn btn-primary btn-small' value='" + RLANG.insert + "'>            <a href='javascript:void(null);' id='redactor_btn_modal_close'>            " + RLANG.cancel + "            </a>          </span>        </div>"
      });
    }
  };
    ...
    Page: {
      Form: {
        init: function() {
          ...
          $('#page_view_name').change(function() {
            return YmCms.Page.Form.showViewTab();
          });
          YmCms.Page.Form.updatePublishedAt();
          return $('#page_published_at').change(function() {
            return YmCms.Page.Form.updatePublishedAt();
          });
        },
        showViewTab: function() {
          var view_name,
            _this = this;
          $('#page_view_name option').not(':selected').each(function(idx, option) {
    ...
    Slideshow: {
      Form: {
        ...
        updateMediaType: function() {
          ...
          return checkedSlideInputs.each(function(idx, element) {
      ...
      init: function(options) {
        ...
        if (slideCount > 0) {
          ...
          $('div.slideshow .slide img:first').load(function() {
            return $('.slideshow_container').css('height', $(this).parent().height());
          });
          if (slideCount > 1) {
            YmCms.Slideshow.intervalTime = options.interval;
            YmCms.Slideshow.resetInterval();
            $('div.slideshow .slide').click(function(event) {
              event.preventDefault();
              return YmCms.Slideshow.nextSlide();
            });
          }
        }
        $(window).resize(function() {
          return YmCms.Slideshow.reAlign();
        });
        return YmCms.Slideshow.reAlign();
      },
      reAlign: function() {
        ...
        return $('.slideshow .slide_inner').css('left', leftPos);
      },
      resetInterval: function() {
        if (YmCms.Slideshow.intervalTime !== void 0) {
          if (YmCms.Slideshow.interval !== void 0) {
            window.clearInterval(YmCms.Slideshow.interval);
          }
          return YmCms.Slideshow.interval = window.setInterval("YmCms.Slideshow.nextSlide()", YmCms.Slideshow.intervalTime);
        }
      },
      nextSlide: function() {
        var currentHeight, nextSlideHeight;
        ...
        if (currentHeight < nextSlideHeight) {
          $('.slideshow_container').animate({
            height: "+=" + (nextSlideHeight - currentHeight)
          }, 500);
        } else {
          $('.slideshow_container').animate({
            height: "-=" + (currentHeight - nextSlideHeight)
          }, 500);
        }
        return $('div.slideshow div.slideshow_inner').animate({
          left: "-=" + YmCms.Slideshow.slideWidth
        }, 500, function() {
          var currentSlide;
          ...
          return YmCms.Slideshow.resetInterval();
        });
      }
    }
  };
...
(function() {
  ...
  YmTags = {
    Input: {
      init: function() {
        return $('input[data-autocomplete]').live('keydown', function(event) {
          if (event.keyCode === $.ui.keyCode.TAB) return event.preventDefault();
        });
      }
    }
  };
...
(function() {
  var YmComments, YmPosts;

  YmPosts = {
    init: function() {
      YmPosts.Form.init();
      return YmPosts.Pagination.init();
    },
    Form: {
      init: function() {
        $('form.post_form:not(.expanded)').live('focusin', function() {
          return $(this).addClass('expanded');
        });
        return $('a[data-toggle]').live('click', function(event) {
          return event.preventDefault();
        });
      }
    },
    Pagination: {
      init: function() {
        return $('.posts .pagination a').live('ajax:before', function() {
          return $('.posts').addClass('loading');
        });
      }
    }
  };
  ...
  YmComments = {
    Form: {
      init: function(options) {
        ...
        $('.comment-form form').live('ajax:beforeSend', function() {
          return $(this).addClass("loading");
        });
        $(".comment-form textarea").autogrow();
        if (options.submitOnEnter) {
          return $(".comment-form textarea").live('keydown', function(event) {
            if (event.keyCode === 13) {
              event.preventDefault();
...
(function() {
  ...
  AbstractChosen = (function() {
    ...
    function AbstractChosen(form_field, options) {
      ...
      this.default_text_default = this.is_multiple ? "Enter some tags" : "Select an option";
      this.setup();
      this.set_up_html();
    ...
    AbstractChosen.prototype.set_default_values = function() {
      ...
      this.create_option_showing = false;
...
(function() {
  ...
  Chosen = (function(_super) {
    ...
    Chosen.prototype.container_mousedown = function(evt) {
      ...
      if (!this.is_disabled) {
        ...
        if (!this.pending_destroy_click && !target_closelink) {
          if (!this.active_field) {
            if (this.is_multiple && (this.search_field.val() === this.default_text_default)) this.search_field.val("");
    ...
    Chosen.prototype.show_create_option = function(terms) {
      ...
      this.create_option_showing = true;
...
(function() {
  ...
  InterestedModal = {
    init: function() {
      ...
      $("#interest-form-submit").click(function() {
        ...
        return $("#interested").modal("hide");
      });
      return $("#availability_datepicker").datepicker({
    ...
    beforeShowDay: function(date) {
      ...
      return [dateAvailable, dateClass, ""];
    },
    onSelect: function(dateText, inst) {
      ...
      return $("#availability_datepicker").datepicker("refresh");
    }
  };
...
(function() {
  ...
  TourForm = {
    ...
    init: function() {
      ...
      if (TourForm.startDate && TourForm.endDate) TourForm.updateAvailability();
      $('.ui-datepicker-booked').tooltip();
      $('#tour_start_on_s,#tour_end_on_s').change(function() {
        return setTimeout("TourForm.updateAvailability(true)", 200);
      });
      $('#popover_external_venue_name').keydown(function(e) {
        $('#popover_venue_id').val(null);
        if (e.keyCode === 13) {
          e.preventDefault();
          return TourForm.savePopover();
        }
      });
      $('#popover_venue_id').change(function() {
        return $('#popover_external_venue_name').val(null);
      });
      $('#popover_save').click(function(e) {
        e.preventDefault();
        return TourForm.savePopover();
      });
      $('#popover_destroy').click(function(e) {
        e.preventDefault();
        return TourForm.destroyBookingPopover();
      });
      return $('#popover_close').click(function(e) {
        e.preventDefault();
        return TourForm.closePopover();
      });
    },
    getStartDate: function() {
      return $.datepicker.parseDate('dd/mm/yy', $('#tour_start_on_s').val());
    },
    getEndDate: function() {
      return $.datepicker.parseDate('dd/mm/yy', $('#tour_end_on_s').val());
    },
    setStartDate: function(date) {
      return $('#tour_start_on_s').val($.datepicker.formatDate('dd/mm/yy', date));
    },
    setEndDate: function(date) {
      return $('#tour_end_on_s').val($.datepicker.formatDate('dd/mm/yy', date));
    },
    availableMonths: function() {
      var endMonth, months, startMonth;
    ...
    beforeShowDay: function(date) {
      var dateClass, dateString, selectable, tooltip, tourDate;
    ...
    onSelect: function(dateText, inst) {
      ...
      if (TourForm.bookingMode) {
        ...
        if (tourDate != null) {
          return setTimeout("TourForm.showPopover('" + dateText + "')", 100);
        }
      } else {
        tourDate = TourForm.dates[dateText];
        if (tourDate != null) {
          return TourForm.removeDate(dateText);
        } else {
          return TourForm.addDate(dateText);
        }
      }
    },
    closePopover: function() {
      ...
      $('.ui-datepicker-booked').tooltip();
      return $('#availability_popover').hide();
    },
    savePopover: function() {
      var dateFields, dateText;
      ...
      if (($('#popover_venue_id').val() != null) && $('#popover_venue_id').val() !== "") {
      ...
      if (!TourForm.bookingsPage) $('form').submit();
      $('#availability_popover').hide();
      return $('#availability_datepicker').datepicker('refresh');
    },
    showPopover: function(dateText) {
      var pos, tourDate;
      $('#availability_popover').hide();
      tourDate = TourForm.dates[dateText];
      if (tourDate.booked) {
      ...
      return $('#availability_popover').show();
    },
    destroyBookingPopover: function() {
      var answer, dateFields, dateText;
    ...
    addDate: function(dateText) {
      var tourDate, tourDateInput;
      tourDate = TourForm.dates[dateText];
      if (tourDate == null) {
        ...
        return TourForm.dates[dateText] = {
          booked: false
        };
      }
    },
    removeDate: function(dateText) {
      var tourDate;
      tourDate = TourForm.dates[dateText];
      if ((tourDate != null) && !tourDate.booked) {
        delete TourForm.dates[dateText];
        return $("#availability .nested-fields:has('input[value=\"" + dateText + "\"]') a.remove_fields").click();
      }
    },
    updateAvailability: function(addDates) {
      var date, datesToAdd, datesToRemove, error, newEndDate, newStartDate, oldEndDate, oldStartDate,
        _this = this;
      ...
      TourForm.startDate || (TourForm.startDate = newStartDate);
      TourForm.endDate || (TourForm.endDate = newEndDate);
      if (!((TourForm.startDate != null) && (TourForm.endDate != null))) return;
      ...
      error = false;
      $.each(datesToRemove, function(index, value) {
        var tourDate;
        tourDate = TourForm.dates[TourForm.dateToString(value)];
        if ((tourDate != null) && tourDate.booked) return error = true;
      });
      if (error) {
        alert("ERROR\nThere are existing tour bookings outside this date range.");
        TourForm.setStartDate(TourForm.startDate);
        return TourForm.setEndDate(TourForm.endDate);
      } else {
        $.each(datesToAdd, function(index, value) {
          return TourForm.addDate(TourForm.dateToString(value));
        });
        $.each(datesToRemove, function(index, value) {
          return TourForm.removeDate(TourForm.dateToString(value));
        });
        ...
        return $('#availability_datepicker').datepicker("option", "numberOfMonths", TourForm.availableMonths());
      }
    }
  };
...
(function() {
  var DEFAULT_LOCATION, VenueBookingForm, VenueLocationMap, VenuesMap;
  ...
  VenueBookingForm = {
    init: function() {
      ...
      $('#popover_save').click(function(e) {
        e.preventDefault();
        return VenueBookingForm.savePopover();
      });
      $('#popover_destroy').click(function(e) {
        e.preventDefault();
        return VenueBookingForm.destroyBookingPopover();
      });
      $('#popover_unavailable').click(function(e) {
        e.preventDefault();
        return VenueBookingForm.makeUnavailable();
      });
      return $('#popover_close').click(function(e) {
        e.preventDefault();
        return VenueBookingForm.closePopover();
      });
    },
    dateToString: function(date) {
      return "" + (('0' + date.getDate()).slice(-2)) + "/" + (('0' + (date.getMonth() + 1)).slice(-2)) + "/" + (date.getFullYear());
    ...
    onSelect: function(dateText, inst) {
      ...
      return setTimeout("VenueBookingForm.showPopover('" + dateText + "')", 100);
    },
    closePopover: function() {
    ...
    savePopover: function() {
      var bookedDate, dateFields, dateText, tour_id;
      dateText = VenueBookingForm.selectedDateText;
    ...
    addDate: function(dateText) {
      ...
      if (tourDate == null) {
        ...
        tourDateInput.val(dateText);
        tourDateInput.attr('value', dateText);
        return VenueBookingForm.dates[dateText] = {
          booked: false
        };
      }
    },
  ...
  VenuesMap = {
    init: function(large) {
      var bounds, mapOptions, marker, venue, _i, _len, _ref;
      large || (large = false);
      mapOptions = {
      ...
      mapOptions.zoom = large ? 6 : 8;
      VenuesMap.map = new google.maps.Map(document.getElementById("map-canvas"), mapOptions);
      VenuesMap.infowindow = new google.maps.InfoWindow({
        maxWidth: 530
      });
      bounds = new google.maps.LatLngBounds;
      _ref = VenuesMap.venues;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        venue = _ref[_i];
        ...
        if (large) {
          ...
          google.maps.event.addListener(marker, 'click', function() {
            ...
            return VenuesMap.infowindow.open(VenuesMap.map, this);
          });
        } else if (VenuesMap.venues.length > 1) {
          google.maps.event.addListener(marker, 'click', function() {
            return window.location = "/venues/" + this.venueId;
          });
        }
      }
      if (VenuesMap.venues.length) {
        return VenuesMap.map.setCenter(bounds.getCenter());
      }
    }
  };

  VenueLocationMap = {
    init: function(lat, lng, notDefault) {
      ...
      mapOptions.zoom = notDefault ? 12 : 8;
      ...
      return google.maps.event.addListener(map, 'center_changed', function() {
        ...
        return VenueLocationMap.update(mapCenter);
      });
    },
    update: function(latLng) {
      $('#venue_lat').val(latLng.lat());
      return $('#venue_lng').val(latLng.lng());
    }
  };
...
(function() {
  ...
  Link = {
    init: function() {
      $('#links .link_url input').live('keyup', function() {
        var host, title, url;
        ...
        if (title) {
          ...
          if (url && host && !host.match(/^(w*|ht?t?p?)$/)) {
            host = host.toLowerCase().replace(/^./, function(letter) {
              return letter.toUpperCase();
            });
            return title.val(host);
          }
        }
      });
      return $('#links .link_title.title_blank  input').live('keyup', function() {
        return $(this).parents('.title_blank').removeClass('title_blank');
      });
    }
  };
  ...
  $(document).ready(function() {
    ...
    $('a[data-scroll]').live('click', function(event) {
      event.preventDefault();
      return $.scrollTo($(this).data('scroll'), {
        duration: 1000
      });
    });
    return $('textarea[maxlength]').live('keyup blur', function() {
      var maxlength, val;
      maxlength = $(this).attr('maxlength');
      val = $(this).val();
      if (val.length > maxlength) return $(this).val(val.slice(0, maxlength));
    });
  });
